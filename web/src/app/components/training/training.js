...
function TrainingController($scope, $rootScope, $interval, $http, NLU_Status, Agent, Intents, Expressions, ExpressionParameters, NLU_Config
  ...
  $scope.getData2 = function(agent_id) {
  ...
  function generateData(intents, expressions, params, synonyms) {
    ...
    var tmpParam = {};
    tmpData.mynlu_data = {}
    tmpData.mynlu_data.common_examples = [];
              ...
              if (synonymList !== undefined) {
                for (synonym_i = 0; synonym_i <= synonymList.length - 1; synonym_i++) {
                  if (synonymList[synonym_i].synonym_reference === tmpParam.value) {
                    ...
                    tmpData.mynlu_data.common_examples.push(tmpSynonymIntent);
                  }
                }
              }
            }
            tmpData.mynlu_data.common_examples.push(tmpIntent);
          }
        }
      }
    }

    for (var i = 0; i <= tmpData.mynlu_data.common_examples.length - 1; i++) {
      var parameterList = params.filter(param => param.expression_id === tmpData.mynlu_data.common_examples[i].expression_id);
      var entities = [];
      if (tmpData.mynlu_data.common_examples[i].entities.length !== parameterList.length) {
          var missingEntities = parameterList.filter(param => param.entity_id != tmpData.mynlu_data.common_examples[i].entities[0].entity_id);
          for (parameter_i = 0; parameter_i <= missingEntities.length - 1; parameter_i++) {
            tmpParam = {};
            var start = tmpData.mynlu_data.common_examples[i].text.indexOf(missingEntities[parameter_i].parameter_value);
            ...
            tmpData.mynlu_data.common_examples[i].entities.push(tmpParam);
          }
      }
      delete tmpData.mynlu_data.common_examples[i].expression_id;
    }
